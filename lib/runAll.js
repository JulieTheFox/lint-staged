...
//this should be somehow passed to vcs workflow for common errors processing
...
const runAll = async (
  ...
  // here could be some factory, by default returning git workflow.
  // repo checks and paths (gitDir, gitConfigDir) belongs to vcsWorkflow entity.
  const vcsWorkflow = await makeVcsWorkflow({ allowEmpty, matchedFileChunks }, symbols);

  //warnings about backup are also git-specific, can be processed in prepare phase
  ctx.shouldBackup = await vcsWorkflow.shouldBackup();

  const files = await vcsWorkflow.getStagedFilesList({ cwd });
  ...
  const runner = new Listr(
    [
      {
        title: 'Preparing...',
        task: (ctx) => vcsWorkflow.prepare(ctx),
      },
      {
        title: 'Hiding unstaged changes to partially staged files...',
        task: (ctx) => vcsWorkflow.hideUnstagedChanges(ctx),
        enabled: hasPartiallyStagedFiles,
      },
      ...listrTasks,
      {
        title: 'Applying modifications...',
        task: (ctx) => vcsWorkflow.applyModifications(ctx),
        skip: applyModificationsSkipped,
      },
      {
        title: 'Restoring unstaged changes to partially staged files...',
        task: (ctx) => vcsWorkflow.restoreUnstagedChanges(ctx),
        enabled: hasPartiallyStagedFiles,
        skip: restoreUnstagedChangesSkipped,
      },
      {
        title: 'Reverting to original state because of errors...',
        task: (ctx) => vcsWorkflow.restoreOriginalState(ctx),
        enabled: restoreOriginalStateEnabled,
        skip: restoreOriginalStateSkipped,
      },
      {
        title: 'Cleaning up...',
        task: (ctx) => vcsWorkflow.cleanup(ctx),
